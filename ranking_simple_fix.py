...
def confirmatory_report(result):
    """Generate a report for the confirmatory analysis results."""
    try:
        report_items = []

        # Extract satisfaction constraint info
        satisfaction_constraint = result.get('satisfaction_constraint')
        constraint_index = result.get('constraint_index')
        satisfaction_level_removed = result.get('satisfaction_level_removed', False)
        
        # Extract key results
        brand_clusters = result.get('brand_clusters', [])
        cluster_to_brand = result.get('cluster_to_brand', {})
        collapsed_pmf = result.get('collapsed_pmf')
        ranking = result.get('ranking')
        col_avg_sat = result.get('col_avg_sat')
        
        # Create list of unique cluster IDs in sorted order
        unique_clusters = sorted(list(set(brand_clusters)))
        # Create mapping from cluster ID to index in collapsed_pmf
        cluster_to_index = {cluster: i for i, cluster in enumerate(unique_clusters)}
        
        # 1. Brand Clusters Section
        report_items.append("# Confirmatory Analysis Results")
        report_items.append("## Brand Clusters")
        
        for cluster_id in sorted(cluster_to_brand.keys()):
            brands = cluster_to_brand[cluster_id]
            if len(brands) == 1:
        ...
        # 2. Satisfaction Levels Section
        report_items.append("## Satisfaction Levels")
        if satisfaction_level_removed:
            if constraint_index == 0:
                report_items.append("The first satisfaction level (Level 1) was omitted during the analysis.")
            else:
                report_items.append("The last satisfaction level was omitted during the analysis.")
        else:
            report_items.append("All satisfaction levels were included in the analysis.")
        
        # 3. Stochastic Ordering Section
        report_items.append("## Stochastic Ordering")
        if ranking:
            report_items.append("Stochastic ordering was found between the brand clusters.")
            
            # Create the ordered ranking table
            ordering_table = []
            
            if "partitioned_satisfaction" in result:
                # Get satisfaction partitions (e.g. [[0], [1,2], [3,4]])
                ...
                # Create satisfaction level labels based on partitions
                level_labels = []
                for partition in partitions:
                    if len(partition) == 1:
                        # Single level
                        level_labels.append(f"Level {partition[0] + 1 + level_offset}")
                    else:
                        # Multiple levels in a partition
                        level_labels.append(f"Levels {partition[0] + 1 + level_offset}-{partition[-1] + 1 + level_offset}")
            else:
                # No partitioning info available, create default labels
                num_cols = collapsed_pmf.shape[1]
                
                # Adjust the level numbers if first level was omitted
                level_offset = 1 if satisfaction_constraint == "First" else 0
                
                level_labels = [f"Level {i + 1 + level_offset}" for i in range(num_cols)]
            
            # Header row with level labels
            ordering_table.append(["Cluster"] + level_labels)
            
            # For each cluster in the ranking order
            for cluster_id in ranking:
                ...
                for col in range(collapsed_pmf.shape[1]):
                    row_data.append(f"{collapsed_pmf[cluster_idx, col]:.4f}")
                
                ordering_table.append(row_data)
            
            # Convert to DataFrame for display
            ordering_df = pd.DataFrame(ordering_table[1:], columns=ordering_table[0])
            report_items.append("### Final Ranking with Stochastic Ordering")
            report_items.append(ordering_df)
        else:
            report_items.append("No stochastic ordering was found between the brand clusters.")
        
        # 4. Detailed Cluster Analysis Section
        report_items.append("## Detailed Cluster Analysis")
        ...
        for cluster_id in sorted(cluster_to_brand.keys()):
            ...
            if "partitioned_satisfaction" in result:
                ...
                # No partitioning info, create default labels
                level_offset = 1 if satisfaction_constraint == "First" else 0
                level_labels = [f"Level {i + 1 + level_offset}" for i in range(collapsed_pmf.shape[1])]
            
            # Create a dictionary for the distribution
            dist_dict = {level: prob for level, prob in zip(level_labels, prob_dist)}
            
            # Convert to a DataFrame for better display
            dist_df = pd.DataFrame([dist_dict])
            report_items.append(dist_df)
        
        # 5. Distributions with Omitted Category Reintegrated (if applicable)
        if "reintegrated_pmf" in result and satisfaction_level_removed:
            report_items.append("## Distributions with Omitted Category Reintegrated")
            
            # Adjust the level numbers if first level was omitted
            level_offset = 1 if satisfaction_constraint == "First" else 0
            
            if "partitioned_satisfaction" in result:
                # Get satisfaction partitions
                partitions = result["partitioned_satisfaction"]
                
                # Create labels for the partitioned levels
                partitioned_labels = []
                for partition in partitions:
                    if len(partition) == 1:
